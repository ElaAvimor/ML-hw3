...
class conditional_independence():
    ...
    def is_X_Y_dependent(self):
        ...
        ###########################################################################
        return not (all(np.isclose(X[x] * Y[y], X_Y[(x, y)]) for (x, y) in X_Y))
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
    ...
    def is_X_Y_given_C_independent(self):
        ...
        # TODO: Implement the function.                                           #
        ###########################################################################
        return all(np.isclose((X_C[(x, c)] * Y_C[(y, c)]) / C[c], X_Y_C[(x, y, c)]) for x in X for y in Y for c in C)
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
...
def poisson_log_pmf(k, rate):
    ...
    # TODO: Implement the function.                                           #
    ###########################################################################
    top = np.power(rate,k) * np.power(np.e, -rate)
    bottom = np.array([np.math.factorial(num) for num in k])
    log_p =  np.log2(top / bottom)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return log_p
...
def get_poisson_log_likelihoods(samples, rates):
    ...
    for rate in rates:
        log_sum = np.sum(poisson_log_pmf(samples, rate))
        results.append(log_sum)  
...
class NaiveNormalClassDistribution():
    def __init__(self, dataset, class_value):
        ...
        self.feature_data = self.filtered_data[:, :-1]

        self.mean = np.mean(self.feature_data, axis = 0)
        self.var = np.mean((self.feature_data - self.mean) ** 2, axis = 0)
    ...
    def get_instance_likelihood(self, x):
        ...
        ###########################################################################
        # likelihood is p(A|x)
        likelihood = np.prod(normal_pdf(x,self.mean,self.std))
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
        return likelihood
